...
class MinkGeM(nn.Module):
    ...
    def forward(self, x: ME.SparseTensor):
        ...
        output = temp.F.pow(1./self.p)    # Return (batch_size, n_features) tensor
        return output



class MinkSpconvGeM(nn.Module):
    def __init__(self, input_dim, p=3, eps=1e-6):
        super(MinkSpconvGeM, self).__init__()
        self.input_dim = input_dim
        # Same output number of channels as input number of channels
        self.output_dim = self.input_dim
        self.p = nn.Parameter(torch.ones(1) * p)
    ...
    def forward(self, x):
        assert isinstance(x, ME.SparseTensor) or isinstance(x, spconv.SparseConvTensor)
        
        if isinstance(x, ME.SparseTensor):
            temp = ME.SparseTensor(x.F.clamp(min=self.eps).pow(self.p), coordinates=x.C)

        elif isinstance(x, spconv.SparseConvTensor):
            temp = ME.SparseTensor(x.features.clamp(min=self.eps).pow(self.p), coordinates=x.indices)

        temp = self.f(temp)      
        output = temp.F.pow(1./self.p)    
        return output
